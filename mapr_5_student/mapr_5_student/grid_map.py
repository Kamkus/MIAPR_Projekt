...
class GridMap(Node):
    ...
    def publish_path(self, path):
        ...
        for p in path:
            pose = PoseStamped()
            pose.pose.position.x = self.map.info.resolution * p[0] + self.map.info.resolution / 2.0
            pose.pose.position.y = self.map.info.resolution * p[1] + self.map.info.resolution / 2.0
